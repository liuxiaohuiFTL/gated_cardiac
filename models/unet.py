...
def get_timestep_embedding(timesteps, embedding_dim):
    ...
    if embedding_dim % 2 == 1:  # zero pad
        emb = torch.nn.functional.pad(emb, (0, 1, 0, 0, 0))
    return emb
...
class Downsample(nn.Module):
    ...
    def forward(self, x):
        if self.with_conv:
            # 这块不知道要不要改成pad = (0, 1, 0, 1, 0, 1) 对三个维度进行填充便于卷积
            pad = (0, 1, 0, 1, 0, 1)
            x = torch.nn.functional.pad(x, pad, mode="constant", value=0)
            x = self.conv(x)
        else:
            x = torch.nn.functional.avg_pool3d(x, kernel_size=3, stride=3)
        return x
...
class DiffusionUNet(nn.Module):
    def __init__(self, config):
        ...
        in_channels = config.model.in_channels * \
            2 if config.data.conditional else config.model.in_channels
